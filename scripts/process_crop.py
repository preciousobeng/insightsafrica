...
def extract_ndvi(hdf_paths: list, bbox: dict) -> np.ndarray:
    ...
    if has_h17 and has_h18:
        # 2×2 mosaic: horizontally concatenate the two vertical column mosaics
        col17 = col_mosaic("h17v07", "h17v08")
        col18 = col_mosaic("h18v07", "h18v08")
        # Pad to equal height if needed (shouldn't differ, but guard it)
        h = max(col17.shape[0], col18.shape[0])
        if col17.shape[0] < h:
            col17 = np.vstack([col17, np.full((h - col17.shape[0], col17.shape[1]), np.nan)])
        if col18.shape[0] < h:
            col18 = np.vstack([col18, np.full((h - col18.shape[0], col18.shape[1]), np.nan)])
        mosaic = np.hstack([col17, col18])
