...
Downloads specific Sentinel-2 L2A bands for each galamsey site,
computes NDVI and NDWI for baseline and recent periods,
generates change detection PNGs and updates galamsey_sites.json.

Bands used:
  B03 (Green, 10m)  — for NDWI
  B04 (Red,   10m)  — for NDVI
  B08 (NIR,   10m)  — for NDVI + NDWI
  B11 (SWIR1, 20m)  — for NDWI alt (resampled to 10m)
...
Usage:
    python scripts/process_galamsey.py
    python scripts/process_galamsey.py --site prestea_bogoso
"""
...
CDSE_NODES_URL   = "https://catalogue.dataspace.copernicus.eu/odata/v1/Products({id})/Nodes"
...
def find_band_in_zip(zf: zipfile.ZipFile, band: str, resolution: str) -> str | None:
    ...
    suffix = f"_{band}_{resolution[1:]}.jp2"  # R10m → 10m, R20m → 20m
...
def download_product(product_id: str, product_name: str, token: str) -> Path:
    ...
    # Use a session so the auth header is kept across same-host redirects.
    # For cross-host redirects (catalogue → download), follow manually.
...
def process_site(site: dict, token: str) -> dict:
    ...
    for period in ("baseline", "recent"):
        ...
        # Save individual period PNGs
        ndvi_path = PROCESSED_DIR / f"galamsey_{sid}_{period}_ndvi.png"
        ndwi_path = PROCESSED_DIR / f"galamsey_{sid}_{period}_ndwi.png"
    ...
    if "baseline" in indices and "recent" in indices:
        ...
        def align_to_baseline(src_arr, src_prof, dst_prof):
            """Reproject src_arr onto the baseline grid."""
        ...
        change_png_path = PROCESSED_DIR / f"galamsey_{sid}_change.png"
    ...
    site.update({
        ...
        "period":      period_label,
        "ndvi_png":    f"galamsey_{sid}_recent_ndvi.png" if "recent" in indices else None,
        "ndwi_png":    f"galamsey_{sid}_recent_ndwi.png" if "recent" in indices else None,
...
def main():
    parser = argparse.ArgumentParser(description="Process Sentinel-2 galamsey change detection")
    ...
    if not sites_path.exists():
        print("galamsey_sites.json not found — run fetch_sentinel2.py first")
        return
    ...
    for site in to_process:
        try:
            updated_by_id[site["id"]] = process_site(site, token)
        except Exception as e:
            print(f"  Error processing {site['id']}: {e}")
            updated_by_id[site["id"]] = site
        time.sleep(2)

    # Merge processed sites back into full list (preserves unprocessed sites)
